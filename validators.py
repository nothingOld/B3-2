...
def parse_commit_result(text: str) -> tuple[str, str]:
    ...
    for line in lines:
        ...
        if stripped_line.startswith(COMMIT_TITLE_PREFIX):
            title = stripped_line.removeprefix(COMMIT_TITLE_PREFIX).strip()
            continue
...
def parse_pr_result(text: str) -> tuple[str, str]:
    ...
    for line in lines:
        ...
        if stripped_line.startswith(PR_TITLE_PREFIX):
            title = stripped_line.removeprefix(PR_TITLE_PREFIX).strip()
            continue
...
def validate_commit_result(text: str) -> list[str]:
    ...
    if not 1 <= bullet_count <= 2:
        errors.append("커밋 본문은 1~2개의 불릿으로 작성해야 합니다.")
...
def validate_pr_result(text: str) -> list[str]:
    ...
    if not title:
        ...
        errors.append(f"PR 제목이 {MAX_PR_TITLE_LENGTH}자를 초과했습니다.")
    ...
    for index, header in enumerate(PR_SECTION_HEADERS):
        ...
        if not has_bullet:
            errors.append(f"{header} 섹션에 불릿이 없습니다.")

    return errors


def validate_result(command: str, text: str) -> list[str]:
